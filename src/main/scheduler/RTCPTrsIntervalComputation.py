...
class RTCPTransmissionIntervalComputation:
    ...
    @staticmethod
    def compute_rtcp_transmission_interval(state: RTCPParticipantState):
        ...
        if(state.senders <= RTCPTransmissionIntervalComputation.SENDER_PERCENT_INCREMENT_SPLITTER * state.members):
            ...
            if(state.we_send):
                ...
                c = state.average_packet_size / (RTCPTransmissionIntervalComputation.SENDER_BANDWIDTH_COEFFICIENT * state.target_bandwidth)
                n = state.senders
            else:
                # The participant is not a sender
                c = state.average_packet_size / (RTCPTransmissionIntervalComputation.RECEIVER_BANDWIDTH_COEFFICIENT * state.target_bandwidth)
                n = state.members - state.senders
        else:
            # The senders are more than 25% of members
            c = state.average_packet_size / state.target_bandwidth
            n = state.members
        t_min = RTCPTransmissionIntervalComputation.T_MIN_INITIALIZED
        if(state.initial):
            t_min = RTCPTransmissionIntervalComputation.T_MIN_NOT_INITIALIZED
